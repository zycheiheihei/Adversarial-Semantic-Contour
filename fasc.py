...
parser.add_argument('--method', type=str, default='fasc', help='using which method')
parser.add_argument('--root', type=str, default='data/demo', help='using which method')


class FixedASC_Attacker(object):
    def __init__(self, method, task, model_type, root):
        ...
        self.output = os.path.join(self.root,'result/'+task+'/'+method+'_'+model_type+'/')
    ...
    def loadMask(self, img_name):
        ...
        maskpath = os.path.join(self.root,self.method+'/'+filename)
...
if __name__ == "__main__":
    ...
    attacker = FixedASC_Attacker(method=args.method, task=args.task, model_type=args.model, root=args.root)
    attacker.train()